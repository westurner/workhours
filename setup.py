...
try:
    ...
    for rtxtpath in rtxtdict:
        requirements = rtxtdict[rtxtpath] = list(
            pip.req.parse_requirements(rtxtpath, session="1")
        )
    ...
    warnings.warn(
        "Pip not found; skipping loading requirements files: %r"
        % requirementstxtfiles
    )
...
setup(
    ...
    package_data={"workhours": ["README.rst", "CHANGES.rst", "scripts/*.sh",]},
    include_package_data=True,
    zip_safe=False,
    entry_points={
        # -*- Entry points: -*-
        "paste.app_factory": ["main = workhours:main"],
        "console_scripts": ["workhours = workhours.climain:main",],
    },
