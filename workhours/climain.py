...
class TestWorkhoursCLI(unittest.TestCase):
    ...
    def test_workhours_main(self):
        ARG_TESTS = (
            ...
            ("--db", "sqlite:///test.db", "--fs", "./tmp", "--print-all"),
            (
                "--db",
                "sqlite:///test.db",
                "--fs",
                ...
                "-s",
                "log.shell",
                os.environ.get("USRLOG", os.path.expanduser("~/.usrlog")),
                "--parse",
                "--report=events",
            ),
        )
...
def CommandlineOptionParser(*args, **kwargs):
    ...
    prs.add_option(
        "-P", "--parse", dest="parse", action="store_true", help="Parse"
    )
    ...
    prs.add_option(
        "-I", "--ipython", dest="ipython", action="store_true",
    )
    prs.add_option(
...
def main(*args):
    ...
    if opts.output is "-":
        ...
        opts._output = opts._output or codecs.open(
            opts.output, "w", encoding="utf8"
        )
    ...
    def read_config_file(config_file, opts=None, log=logging, **kwargs):
        ...
        opts.fs_url = get_value(_config, "fs_url", ("main", "fs.url"), opts)
        opts.sqldb_url = get_value(
            _config, "sqldb_url", ("main", "db_main.url"), opts
        )
        opts.esdb_url = get_value(
            _config, "esdb_url", ("main", "esdb.url"), opts
        )
    ...
    if opts.parse:
        if not opts._queues:
            prs.error(
                "--parse specified without any queues (see -c and/or -s)"
            )
            # raise Exception()
        if any(x[0] for x in opts._queues.items()):
            ...
            for result in events_table_worker(
                opts.sqldb_url, opts._queues, filestore=opts.fs_url
            ):
