...
def setup_mappers(meta=None, engine=None):
    ...
    users_tbl = Table(
        ...
        Column(
            "password", PasswordType(schemes=["pbkdf2_sha512",]), nullable=True
        ),
    ...
    tasksources_tbl = Table(
        ...
        Column(
            "type", Unicode(length=255), index=True
        ),  # taskqueue.type # TODO
    ...
    tasks_tbl = Table(
        ...
        Column("id", GUID(), primary_key=True, nullable=False),
        Column(
            "source_id", GUID(), ForeignKey(tasksources_tbl.c.id), index=True
        ),
        Column("args", MutationDict.as_mutable(JSONEncodedDict)),
        Column("label", UnicodeText()),
        Column(
            "state", Unicode(), index=True, nullable=True
        ),  # TODO: ENUM: celery
        Column("statemsg", Unicode()),
        Column(
            "date", DateTime(), index=True, onupdate=datetime.datetime.now,
        ),
        Column("created_on", DateTime, default=datetime.datetime.now),
        Column("updated_on", DateTime, onupdate=datetime.datetime.now),
    )
    mapper(
        Task,
        tasks_tbl,
        properties={"source": relation(TaskSource, backref="tasks"),},
    )
    ...
    mapper(
        Report,
        reports_tbl,
        properties={"report_type": relation(ReportType, backref="reports"),},
    )
