...
def _connect_sqldb(request):
    ...
    if sql_url:
        engine = engine_from_config(request.registry.settings, "db_main.")
        ...
        def setup(*args):
            ...
            return initialize_sql(
                engine=engine, setup_mappers=setup_mappers, Base=Base
            )
...
def config_db(config, settings, prefix="sqlalchemy."):
    ...
    config.add_request_method(
        _connect_sqldb, "db_session", property=True, reify=True
    )
    config.add_request_method(
        _connect_esdb, "es_session", property=True, reify=True
    )
...
def config_routes(config):
    ...
    config.add_restful_routes(
        "api/events", EventsContextFactory, view=EventsRESTfulView
    )
    ...
    config.add_restful_routes(
        "api/places", PlacesContextFactory, view=PlacesRESTfulView
    )
    ...
    config.add_restful_routes(
        "reports/domains", DomainsContext, view=DomainsView
    )
    config.add_restful_routes(
        "reports/projects", ProjectsContext, view=ProjectsView
    )
    config.add_restful_routes(
        "reports/wikipedia", WikipediaPagesContext, view=WikipediaPagesView
    )
...
def config_error_pages(config):
    config.add_view(
        "workhours.site.views.errors.http404",
        renderer="workhours:templates/http404.jinja2",
        context="pyramid.exceptions.NotFound",
    )

    config.add_view(
        "workhours.site.views.errors.http403",
...
def config_secrets(settings):
    if "secrets" in settings:
        try:
            ...
            log.warn(
                "secrets were specificed in the configuration but could not be read\n\n%s"
                % settings.get("secrets", ""),
                exc_info=1,
            )
