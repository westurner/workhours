...
class ProjectsContext(ReportContext):
    ...
    RESERVED_WORDS = dict(
        (x, 1)
        for x in ("settings", "session", "account", "login", "users", "repo")
    )
    ...
    def generate_report(self):
        query = (
            ...
            .filter(
                Place.netloc.in_(
                    ("bitbucket.org", "github.com", "code.google.com",)
                )
            )
        )

        def mapfn(obj):
            return ((obj.netloc,) + tuple(obj.path.split("/")[1:]), obj)
...
class WikipediaPagesContext(ReportContext):
    ...
    def generate_report(self):
        query = (
            self.request.db_session()
            .query(Place)
            .filter(Place.netloc.like("%wikipedia.org"))
        )
